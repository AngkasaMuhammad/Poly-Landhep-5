...
'vertex_struct',async ({
	...
	let buf = (await reso.get(descr.array)).buffer
	let offset = descr.offset
	let len = descr.length
	let lay = descr.layout
	let st = []
	...
	for(let i = 0;i < len;i++){
		...
		for(let prop in lay){
			let {
				len:flen,
				type:ftype,
			} = vertformat[lay[prop]] //format
			inst[prop] = new ftype(
				buf,
				memcurlen,
				flen,
			)
			memcurlen += flen*ftype.BYTES_PER_ELEMENT
		}
		st.push(inst)
	}
...
let vertformat = {
	'float32':{ len:1, type:Float32Array, },
	'float32x2':{ len:2, type:Float32Array, },
	'float32x3':{ len:3, type:Float32Array, },
	'float32x4':{ len:4, type:Float32Array, },
	
	'sint32':{ len:1, type:Int32Array, },
	
	'unorm8x4':{ len:4, type:Uint8Array, },
}
...
'wasm_memory',async ({
	...
	arrbuf = await arrbuf.arrayBuffer()
	let pages = Math.max(
		Math.ceil(arrbuf.byteLength / 65536),
		descr.pages ?? 1,
		1,
	)
